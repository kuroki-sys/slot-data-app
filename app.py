...
def parse_page_title(raw):
    ...
    m = re.match(
        r"(\d{4})/(\d{2})/(\d{2})\s+(.+?)\s+データまとめ",
        title,
    )

    if not m:
        raise ValueError(
            f"ページタイトルから日付・店舗名を判定できません: {title}"
        )
...
def parse_anaslo_json(raw):
    ...
    summary_table = next(
        (
            t
            for t in raw.get("tables", [])
            if "total_get_medals_table"
            in str(t.get("class") or "")
        ),
        None,
    )
    ...
    if (
        summary_table
        and len(summary_table.get("rows", [])) >= 2
    ):
        ...
        avg_diff = (
            round(total_diff / machine_count)
            if machine_count
            else None
        )
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                DO UPDATE SET
                    ...
                    total_diff_medals =
                        EXCLUDED.total_diff_medals,

                    avg_diff_medals =
                        EXCLUDED.avg_diff_medals,
            ...
            machine_sql = """
                ...
                DO UPDATE SET
                    ...
                    combined_rate_text =
                        EXCLUDED.combined_rate_text,

                    bb_rate_text =
                        EXCLUDED.bb_rate_text,

                    rb_rate_text =
                        EXCLUDED.rb_rate_text,
...
def date_range_selector(
    ...
    min_date = bounds.iloc[0][
        "min_date"
    ]

    max_date = bounds.iloc[0][
        "max_date"
    ]
...
if menu == "ダッシュボード":
    ...
    summary = query_df(
        ...
        SELECT
            ...
            SUM(total_diff_medals)
                AS total_diff,
            SUM(
                CASE
                    WHEN total_diff_medals > 0
                    THEN 1
                    ELSE 0
                END
            ) AS plus_days,
    ...
    c2.metric(
        "登録日数",
        f"{int(summary['registered_days']):,}日",
    )

    c3.metric(
        "台別データ",
        f"{int(machine_count):,}件",
    )

    c4.metric(
        "期間総差枚",
        f"{int(summary['total_diff'] or 0):+,}枚",
    )
    ...
    c5.metric(
        "プラス日",
        f"{int(summary['plus_days'] or 0)}日",
    )

    c6.metric(
        "マイナス日",
        f"{int(summary['minus_days'] or 0)}日",
    )
